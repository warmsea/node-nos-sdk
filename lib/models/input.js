...
var Input = (function() {
  ...
  function Input(http, rules, payload) {
    ...
    w.each(this.members, function(member, name) {
      if (member.defaultValue !== undefined) {
        this._pushParam(name, undefined);
      }
    }, this);
  }
  ...
  w.extend(Input.prototype._checkType, {
    ...
    boolean: function(param, rule, name) {
      if (param !== true && param !== false) {
        this._pushError('%s: should be boolean value true or false.', name);
      }
    },
    ...
    Standard: function(param, rule, name) {
      switch (rule.location) {
        case 'header':
          ...
          break;
        case 'query':
          this._result.qs[rule.locationName] = param;
          break;
        case 'uri':
